...
#ifndef INC_20240306_INFEXPECTATION_COMPUTATION_INFGRAPH_H
#define INC_20240306_INFEXPECTATION_COMPUTATION_INFGRAPH_H

#endif //INC_20240306_INFEXPECTATION_COMPUTATION_INFGRAPH_H
...
public:
    ...
    InfGraph(string folder, string graph_file, string per):Graph(folder, graph_file, per){
        hyperG.clear();
        for(int i=0; i<n; i++)
        ...
        while(!q.empty()) {
            ...
            if(influModel==IC){
                ...
                for(int j=0; j<(int)gT[i].size(); j++){
                    ...
                    //cout << "gT size: " << gT[i].size() << "\tprobT size: " << probT[i].size() << endl;
                    if(randDouble > probT[i][j]){
                        //cout << "probT: " << probT[i][j] << endl;
                        continue;
                    }
